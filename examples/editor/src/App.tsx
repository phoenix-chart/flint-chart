...
/**
 * Live editor — Vega-Lite-style playground for flint-chart.
 *
 * Left pane: editable JSON for a ChartAssemblyInput.
 * Right pane: tabbed view of each backend's rendered output, plus the
 *   compiled spec.
 */
export function App() {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', height: '100vh' }}>
      ...
      <section style={{ display: 'flex', flexDirection: 'column', borderRight: '1px solid #e1e4e8' }}>
        ...
        >
          <strong>flint-chart · editor</strong>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 16 }}>
          {compiled.ok ? (
            <>
              ...
              {backend === 'echarts' && <EChartsView option={compiled.value} />}
              {backend === 'chartjs' && <ChartjsView config={compiled.value} />}
              {showSpec && (
                <pre
                  style={{
                    marginTop: 16,
